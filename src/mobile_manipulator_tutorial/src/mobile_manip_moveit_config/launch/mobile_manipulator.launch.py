...
def generate_launch_description():
    ...
    os.environ["GZ_SIM_RESOURCE_PATH"] = os.environ.get("GZ_SIM_RESOURCE_PATH", "") + os.pathsep + pkg_navigation_stack + "/gazebo_models/"
    os.environ["GZ_IP"] = "127.0.0.1"
    os.environ["GZ_SIM_SYSTEM_PLUGIN_PATH"] = "/opt/ros/jazzy/lib:/opt/ros/jazzy/lib/gz_ros2_control"
    ...
    spawn_urdf_node = Node(
        package="ros_gz_sim",
        executable="create",
        arguments=["-name", "mobile_manipulator", "-topic", "robot_description", "-x", LaunchConfiguration('x'), "-y", LaunchConfiguration('y'), "-z", "0.5", "-Y", LaunchConfiguration('yaw')],
        output="screen",
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}]
    )

    spawn_urdf_node_delayed = TimerAction(period=7.0, actions=[spawn_urdf_node])
    ...
    launchDescriptionObject.add_action(spawn_urdf_node_delayed)
