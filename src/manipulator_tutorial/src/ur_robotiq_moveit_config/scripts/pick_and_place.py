...
class Commander(Node):
    ...
    def publish_target(self):
        self.get_logger().info('Waiting for ROS 2 DDS connection to establish...')
        import time
        while self.pub.get_subscription_count() == 0:
            time.sleep(0.5)
            self.get_logger().info('Still waiting for ur_robotiq_controller.py to be ready...')
        self.get_logger().info('Connection established! Publishing target.')
